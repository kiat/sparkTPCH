...
public class GenerateDataFileOnHDFS_Dataset {
	...
	public static void main(String[] args) throws FileNotFoundException, IOException {
		...
		SparkSession spark = SparkSession
				...
				.appName("ComplexObjectManipulation_Dataset_add")
				.config("spark.io.compression.codec", "lzf") // snappy, lzf, lz4
																// -
																// Default=snappy
				// .config("spark.speculation", "true")
				.config("spark.shuffle.spill", "true")

				// just in case that you want to run this on localhost in
				// stand-alone Spark mode
				// .master("local[*]")
				.getOrCreate();
...
//		JavaRDD<Customer> customerRDD = customerRDD_raw;
		...
		for (int i = 0; i < NUMBER_OF_COPIES+1; i++) {
			...
			if (numberOfCopies_set.contains(i)) {
				...
				customerDS = customerDS.coalesce(numPartitions);
//				customerDS.saveAsObjectFile(hdfsNameNodePath + i);
			}
